...
#
SUBDIRS := Filters BackgroundStacks MCOnly #ADD_NEW_SUBDIR ... do not remove this comment from this line
...
all:
	@for i in $(SUBDIRS); do ( echo "" && echo "Compiling $$i..." && cd $(LARLITE_USERDEVDIR)/CCpi0Analysis/$$i && $(MAKE) ) || exit $$?; done
#####################################################################################
#
# CLEANs...
#
clean:
	@for i in $(SUBDIRS); do ( echo "" && echo "Cleaning $$i..." && cd $(LARLITE_USERDEVDIR)/CCpi0Analysis/$$i && $(MAKE) clean && rm -f $(LARLITE_LIBDIR)/$$i.* ) || exit $$?; done
...
doxygen:
	...
	@mkdir -p $(LARLITE_USERDEVDIR)/CCpi0Analysis/doc/dOxygenMyProject
	@doxygen $(LARLITE_USERDEVDIR)/CCpi0Analysis/doc/doxygenMyProject.script
